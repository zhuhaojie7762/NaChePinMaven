...
@Controller
@Scope("prototype")
public class SettlementCurController extends BaseController {
    ...
    public void index() throws ParseException {
        ...
        if (StrKit.notBlank(value, fieldName)) {
            if ("phone".equals(fieldName)) {
                ...
                code = value;
                customer = customerService.getCustomerByCode(code);
        ...
        if (customer != null) {
            MonthlyBill bill = monthlyBillDao.calMonthBill(customer.getId(), month, financeEndDay);
            bill.setSellerCode(customer.getCode());
            bill.setSupplierCompany(customer.getSupplierCompany());
            setAttr("bill", bill);
        }
        render("cur_list.html");
    }

    /**
     * 进货商预存款账户打款确认
     */
    @ActionKey("dakuanqueren.html")
    public void dakuanqueren(){render("dakuanqueren.html");}
