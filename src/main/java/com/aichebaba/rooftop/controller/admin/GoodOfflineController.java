...
@Controller
@Scope("prototype")
public class GoodOfflineController extends BaseController {
    ...
    public void index() {
        ...
        if (StrKit.notBlank(fieldName, value)) {
            if ("code".equals(fieldName)) {
                ...
                sellerId = Integer.valueOf(value);
            }
        }
        ...
        PageList<Goods> pager = goodsService
                .findGoods(goodsCode, name, itemNo, isSpecial, sellerId, GoodsStatus.offline, goodsType, getPageParam());
        setAttr("pager", pager);
        render("offline.html");
    }
    ...
    public void applyOnline() {
        int id = getParaToInt("id", 0);
		goodsService.applyOnline(id, getCurUserId());
        ok("已申请");
    }
}
