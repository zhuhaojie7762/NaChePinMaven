...
public class PurchaseAuditController extends BaseController {
    ...
    public void index() {
        ...
        PageList<CustomerApplyAudit> pager = customerService.findWaitAuditPurchase(phone, code, name, status, getPageParam());
        setAttr("statusList", CustomerAuditState.values());
        setAttr("pager", pager);
        render("list.html");
    }

    public void toAudit() throws UnsupportedEncodingException {
    ...
    public void audit() {
        ...
        if (sign == null || sign.length == 0) {
            customerService.auditApplyPurchaseAudit(id, auditResult, reason, getCurUser(), null);
        }else {
            customerService.auditApplyPurchaseAudit(id, auditResult, reason, getCurUser(), Joiner.on(";").join(sign));
        }
        ok("操作成功");
    }
